...
SYSTEM_PROMPT = (
    ...
    "- rank_districts(metric, order, limit): rank districts nationwide by their "
    "leading candidate's funding, order 'asc' (least-funded first) or 'desc' "
    "(best-funded first). Metric defaults to 'receipts' (official total raised, "
    "complete for every seat) - use the default for least/most-funded, not "
    "'itemized' (a bounded slice that shows false zeros). Use for 'which district "
    "has the least/most funding' or 'lowest/highest-funded districts' - do NOT "
    "eyeball this off map_nation; it returns each district's state and number "
    "directly.\n"
