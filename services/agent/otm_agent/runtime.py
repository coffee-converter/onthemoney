...
SYSTEM_PROMPT = (
    ...
    "- rank_districts(metric, order, limit): rank districts nationwide by TOTAL "
    "funding summed across the seat's candidates, order 'asc' (least-funded first) "
    "or 'desc' (best-funded first); value is the district total and the named "
    "candidate is the seat's top raiser. Metric defaults to 'receipts' (official "
    "total raised, complete for every seat) - use the default for least/most-"
    "funded, not 'itemized' (a bounded slice). Use for 'which district has the "
    "least/most funding' or 'lowest/highest-funded districts' - do NOT eyeball "
    "this off map_nation; it returns each district's state and number directly.\n"
    "- compare_candidates(cand_ids): two to four candidates side by side on "
    "receipts, individual money, itemized sum, small-dollar share, out-of-state "
    "share, and top industry. Resolve names to ids with find_candidate first. Use "
    "for 'compare X and Y' or 'is A or B more grassroots-funded'.\n"
    "- top_by_industry(industry, limit): nationwide, the candidates who raise the "
    "most itemized money from an industry (Technology, Finance, Law, Healthcare, "
    "Energy, Real Estate, Education, Government / Public, Retired / Not employed). "
    "Use for 'who gets the most tech/oil/pharma money'.\n"
    ...
    "- map_districts(order, metric, limit): map the least- (order 'asc') or most- "
    "(order 'desc') funded districts nationwide as shaded, labeled polygons. Use "
    "this (never hand-build render_map) for 'map the lowest/highest-funded "
    "districts'; for a single district use highlight_district instead.\n"
    ...
    "numbers.\n"
    "When the answer is a single district or candidate - including a superlative like "
    "the most- or least-funded district - finish by mapping just that one seat with "
    "highlight_district (or emit_scene for a funding view), not a nationwide map, so "
    "the map matches the answer.\n"
    "When you report a LEAST-funded result (a lowest district or the bottom of a "
    "ranking), add one brief caveat: an unusually low total can reflect an FEC "
    "filing or committee-to-candidate linkage gap for that seat (a real candidate "
    "whose committee is not linked in the bulk data) rather than a genuinely cheap "
    "race, so treat the figure as a floor, not a settled fact. Most-funded results "
    "need no such caveat.\n"
