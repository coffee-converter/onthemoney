...
SYSTEM_PROMPT = (
    ...
    "- render_map(points, title): a custom point map you compose; each point has a "
    "place (district id like AZ-01 or state code), value (marker size), color (hex), "
    "label, and tooltip lines. Use for anything the district map cannot show, e.g. a "
    "whole state colored by party and sized by money (gather it with state_field "
    "first).\n\n"
